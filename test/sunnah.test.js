...
test("getting sunnah times for the New York timezone", () => {
	var coords = new adhan.Coordinates(35.7750, -78.6336);
	var params = adhan.CalculationMethod.NorthAmerica();

	var date1 = new Date(2015, 6, 12);
    var p1 = new adhan.PrayerTimes(coords, date1, params);
	expect(moment(p1.maghrib).tz("America/New_York").format("M/D/YY, h:mm A")).toBe("7/12/15, 8:32 PM");
	
	var date2 = new Date(2015, 6, 13);
    var p2 = new adhan.PrayerTimes(coords, date2, params);
	expect(moment(p2.fajr).tz("America/New_York").format("M/D/YY, h:mm A")).toBe("7/13/15, 4:43 AM");
	
	var sunnah = new adhan.SunnahTimes(p1);
...
test("getting sunnah times for the London timezone", () => {
	var coords = new adhan.Coordinates(51.5074, -0.1278);
...
test("getting sunnah times for the Oslo timezone", () => {
	var coords = new adhan.Coordinates(59.9094, 10.7349);
...
test("getting sunnah times for US DST change", () => {
	var coords = new adhan.Coordinates(37.7749, -122.4194);
	var params = adhan.CalculationMethod.NorthAmerica();

	var date1 = new Date(2017, 2, 11);
    var p1 = new adhan.PrayerTimes(coords, date1, params);
...
test("getting sunnah times for Europe DST change", () => {
	var coords = new adhan.Coordinates(48.8566, 2.3522);
	var params = adhan.CalculationMethod.MuslimWorldLeague();
	params.highLatitudeRule = adhan.HighLatitudeRule.SeventhOfTheNight;

	var date1 = new Date(2015, 9, 24);
    var p1 = new adhan.PrayerTimes(coords, date1, params);
