import {dateByAddingDays, roundedMinute} from '../src/DateUtils'
import {
  degreesToRadians,
  normalizeToScale,
  quadrantShiftAngle,
...
describe('TimeComponents', () => {
  ...
  test('utcDate returns an expected date', () => {
    const date = new TimeComponents(1.0084).utcDate(2020, 0, 1)
    expect(date).toEqual(new Date(Date.UTC(2020, 0, 1, 1, 0, 30)))
  })

  test('toSeconds, returns the number of seconds', () => {
    const time = new TimeComponents(1.0084)
    expect(time.toSeconds()).toBeCloseTo(3630, 1)
  })

  test('toHours, returns the number of hours', () => {
    const time = new TimeComponents(1.0084)
    expect(time.toHours()).toBeCloseTo(1.0084, 3)
  })

  test('greaterThan, returns true if the time is greater than the specified time', () => {
    const time1 = new TimeComponents(1.0084)
    const time2 = new TimeComponents(1.0083)
    expect(time1.greaterThan(time2)).toBe(true)
  })

  test('smallerThan, returns true if the time is smaller than the specified time', () => {
    const time1 = new TimeComponents(1.0084)
    const time2 = new TimeComponents(1.0083)
    expect(time2.smallerThan(time1)).toBe(true)
  })

  test('addSeconds, adds the specified number of seconds to the time', () => {
    const secs = 1 + 30 / 3600
    expect(secs).toBe(1.5)
    const time1 = new TimeComponents(1 + 30 / 3600)
    expect(time1.seconds).toBe(30)
    const time2 = TimeComponents.addSeconds(time1, 20)
    expect(time2.seconds).toBeCloseTo(50, 1)
  })

  test('addMinutes, adds the specified number of minutes to the time', () => {
    const time1 = new TimeComponents(1 + 30 / 60)
    const time2 = TimeComponents.addMinutes(time1, 20)
    expect(time2.minutes).toBeCloseTo(50, 1)
  })
})
